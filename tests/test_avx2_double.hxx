// Copyright (c) 2019-2021 Christopher M. Brandt
// All rights reserved

#ifndef NOLA_SIMD_TEST_AVX2_DOUBLE_HXX
#define NOLA_SIMD_TEST_AVX2_DOUBLE_HXX


int test_avx2_double_length(int);
int test_avx2_double_set_zero_and_store(int);
int test_avx2_double_set_scalar(int);
int test_avx2_double_broadcast(int);
int test_avx2_double_load(int);
int test_avx2_double_store(int);
int test_avx2_double_add(int);
int test_avx2_double_sub(int);
int test_avx2_double_mul(int);
int test_avx2_double_div(int);
int test_avx2_double_fma(int);
int test_avx2_double_reduce(int);

int test_avx2_double();


#endif